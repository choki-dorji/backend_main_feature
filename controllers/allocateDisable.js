...
exports.DisableAllocation = async (req, res, next) => {
  ...
  let allocationexist;
  try{
    allocationexist = await Allocate.find({sid: sid, academicyear: currentYear})
  }catch(e){
    return res
      .status(400)
      .json({ message: "couldnot find allocation" });
  }
  if(allocationexist.length > 0){
    return res
      .status(400)
      .json({ message: "allocation exist already for given student cannot allocate for this year " });
  }
  ...
  try{
    ...
    return res
      .status(400)
      .json({ message: "couldnot find student" });
  }
  ...
  disabled = disstudent.filter((s) => String(s.sid) === String(sid) && s.disability === true);
  ...
  try{
    ...
    return res
      .status(400)
      .json({ message: "No room found with provided id" });
  }
  if(!rooms){
    return res
      .status(400)
...
let blocks
  try{
    ...
    return res
      .status(400)
      .json({ message: "No Block found with provided id" });
  }
